...
SCRAPING_RESULT_DIR = '6.Scraping_Result'
...
def scrape_files(directory, pattern):
    """
    Scrape text from all files in the specified directory and its subdirectories
    that matches a given pattern.

    :param directory: Directory to search for files
    :param pattern: Regular expression pattern to search within files
    :return: Dictionary with file names as keys and lists of matched lines as values
    """
    result = {}
    ...
    for file_path in tqdm(file_paths, desc="Processing files", unit="file"):
        ...
        if matched_lines:
            result[file_path] = matched_lines
    ...
    while True:
        ...
        menu_questions = [
            inquirer.List('action',
                          ...
                          choices=['Decompile APK', 'Recompile APK', 'Generate Keystore', 'Sign APK', 'Scrape Files', 'Exit'],
                          ),
        ]
        ...
        if menu_answers['action'] == 'Decompile APK':
        ...
        elif menu_answers['action'] == 'Scrape Files':
            pattern = input("Enter the text pattern to search for (regex supported): ").strip()
            print("Scraping files in '2.Decomple_File', please wait...")

            if not os.path.exists(SCRAPING_RESULT_DIR):
                os.makedirs(SCRAPING_RESULT_DIR)

            results = scrape_files(DECOMPILE_DIR, pattern)

            results_file = os.path.join(SCRAPING_RESULT_DIR, 'search_results.txt')
            ...
            print(f"Scraping completed. Results are saved in {results_file}.")
...
if __name__ == '__main__':
    ...
    os.makedirs(SCRAPING_RESULT_DIR, exist_ok=True)
